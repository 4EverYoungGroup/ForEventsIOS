//
//  DownloadAssistsInteractorNSURLSessionImpl.swift
//  ForEvents
//
...
class DownloadAssistsInteractorNSURLSessionImpl: DownloadAssistsInteractor {
    func execute(onSuccess: @escaping (Events) -> Void, onError: errorClosure) {
        ...
        urlComponents.path = Constants.urlTransactionsListPath
            ...
            OperationQueue.main.addOperation {
                ...
                if error == nil {
                    ...
                    if (self.checkStatusCode(response: response)) {
                        let events = parseTransList(data: data!)
                        //return Movements
                        onSuccess(events)
                    } else {
                        let events = Events()
